...
public class UsuarioDtoConverter {
    ...
    public Usuario transformNewAdmin(UsuarioDto dto) {
        ...
        if (dto.getCentrosDeTrabajoIds() != null) {
            List<CentrosDeTrabajo> centrosDeTrabajo = new ArrayList<>();
            for (Long id : dto.getCentrosDeTrabajoIds()) {
                CentrosDeTrabajo centro = new CentrosDeTrabajo();
                centro.setId(id);
                centrosDeTrabajo.add(centro);
            }
...
//				roles.add(rolService.findByRolNombre(RolNombre.ROLE_ADMIN).get());
    ...
    public UsuarioDto inverseTransform(Usuario u) {
		return UsuarioDto
				...
				.deBaja(u.getDeBaja())
				.centrosDeTrabajoIds(u.getCentrosDeTrabajo() != null ? 
					u.getCentrosDeTrabajo().stream().map(CentrosDeTrabajo::getId).collect(Collectors.toList()) : null)
				.build();
	}
...
//		u.setPassword(passwordEncoder.encode(editar.getPassword()));
		...
		if (editar.getCentrosDeTrabajoIds() != null) {
			List<CentrosDeTrabajo> centrosDeTrabajo = new ArrayList<>();
			for (Long id : editar.getCentrosDeTrabajoIds()) {
				CentrosDeTrabajo centro = new CentrosDeTrabajo();
				centro.setId(id);
				centrosDeTrabajo.add(centro);
			}
			u.setCentrosDeTrabajo(centrosDeTrabajo);
		}
